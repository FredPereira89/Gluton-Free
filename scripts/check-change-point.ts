// Read-only proposal diagnostic. Usage: npx tsx --env-file=.env.local scripts/check-change-point.ts <slug> [YYYY-MM]
...
const diagnosticMonth = process.argv[3];
if (diagnosticMonth && !/^\d{4}-(0[1-9]|1[0-2])$/.test(diagnosticMonth)) throw new Error("diagnostic month must be YYYY-MM");
...
try {
  ...
  if (diagnosticMonth) {
    const candidates = rows.filter((r) => (r.published_at as Date).toISOString().startsWith(diagnosticMonth) && r.text);
    ...
    diagnostic = { month: diagnosticMonth, extracted: extracted.size, mentions: [...extracted.values()].filter((r) => r.change !== "none").length,
