...
function facts(name: string, formatName: string, r: Rollup): string {
  ...
  if (r.redFlags.length) {
    for (const g of r.redFlags) {
      lines.push(`Red flag (${g.group}): ${g.incidents12m} confirmed first-hand incident(s) in the last 12 months, newest ${g.newestAt?.slice(0, 7)}, types ${g.types.join(", ")}; ${g.forcesAvoid ? "forces Avoid" : "shown, does not move the Tier"}.`);
    }
  } else {
